...
void hora_atual(char *buf, int tam)
{
    time_t t = time(NULL);
    struct tm tm_local;
...
void data_hora(time_t t, char *buf, int tam)
{
    struct tm tm_local;
    localtime_r(&t, &tm_local);
...
void processar_linha(const char *linha, int contador, char *sensor_id_out, int id_tam)
{
    char id[64], hora[16];
    float temp, luz, umid;
...
void registrar_sessao(const char *sensor_id, const char *ip, int porta,
...
void atender_cliente(int cliente_fd, struct sockaddr_in *cliente_addr)
{
    ...
    while (!parar) {
        ...
        for (int i = 0; i < n; i++) {
            if (buffer[i] == '\n') {
                ...
                lin_len = 0;
            } else if (lin_len < (int)(sizeof(linha) - 1)) {
    ...
    printf("[%s] [pid %d] Cliente %s:%d desconectado (%d mensagens, %lds) -> %s\n",
           hora, getpid(), ip, porta_cliente,
           contador, fim - inicio, ARQUIVO_LOG);
    fflush(stdout);
...
int main(int argc, char *argv[])
{
    ...
    if (bind(servidor_fd, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) return 1;
    if (listen(servidor_fd, 8) < 0) return 1;
    ...
    while (!parar) {
        ...
        int cliente_fd = accept(servidor_fd, (struct sockaddr *)&cliente_addr, &cliente_len);
